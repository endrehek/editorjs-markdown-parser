...
export async function parseToBlocks(content) {
  ...
  //   .replaceAll(/(\| *)[^a-zA-Z0-9\s-]+/g, '| ') // Remove styling inside tables from source text
...
export default class MarkdownImporter {
  ...
  */
  async parse(markdownData) {
    ...
    const toBlocksData = await parseToBlocks(markdownData); // parse the markdown file to JSON
